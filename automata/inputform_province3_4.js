...
(async () => {
    ...
    try {
        ...
        // 8️⃣ คลิกเมนู "3.4 ระดับความสำเร็จของการจัดทำและขับเคลื่อนแผนปฏิบัติการด้านการพัฒนาสังคมจังหวัด ประจำปีงบประมาณ พ.ศ. 2568"
        await page.waitForSelector('//button[contains(., "3.4 ระดับความสำเร็จของการจัดทำและขับเคลื่อนแผนปฏิบัติการด้านการพัฒนาสังคมจังหวัด ประจำปีงบประมาณ พ.ศ. 2568")]');
        await page.click('//button[contains(., "3.4 ระดับความสำเร็จของการจัดทำและขับเคลื่อนแผนปฏิบัติการด้านการพัฒนาสังคมจังหวัด ประจำปีงบประมาณ พ.ศ. 2568")]');

        // 9️⃣ รอให้เมนูย่อยแสดง แล้วคลิก "บันทึกข้อมูลครั้งที่ 1"
        await page.waitForSelector('a[href="https://volunteer-smart-beta.nu.ac.th/beta-inspectorNew/index.php/Y2568Report0304Controller?time_count=1"]'); // รอให้ลิงก์โหลด
        await page.click('a[href="https://volunteer-smart-beta.nu.ac.th/beta-inspectorNew/index.php/Y2568Report0304Controller?time_count=1"]'); // คลิกลิงก์
        ...
        const categories = [
            { subIndex: 1, field: "children_and_youth" }, // เด็กและเยาวชน
            { subIndex: 2, field: "women_and_family" }, // สตรีและครอบครัว
            { subIndex: 3, field: "persons_with_disabilities" }, // คนพิการ
            { subIndex: 4, field: "elderly" }, // ผู้สูงอายุ
            { subIndex: 5, field: "homeless_and_beggars" }, // คนไร้ที่พึ่ง/ขอทาน
            { subIndex: 6, field: "working_age" }, // วัยแรงงาน
            { subIndex: 7, field: "others" }, // อื่น ๆ(โปรดระบุ)
        ];
        
        for (const { index, number } of data) {
            for (const { subIndex, field } of categories) {
                const selector = `input[name="list[${index}][sub_list][${subIndex}][${field}]"]`;
                await page.waitForSelector(selector);
                await page.fill(selector, number);
            }
        }
        ...
        await page.fill('input[name="position_name_reporter"]', 'ผู้รายงาน3.4'); // ตำแหน่ง
