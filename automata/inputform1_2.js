...
(async () => {
    ...
    try {
        ...
        // 8️⃣ คลิกเมนู
        await page.waitForSelector('//button[contains(., "1.2 โครงการขยายผลโครงการบริบาลและคุ้มครองสิทธิผู้สูงอายุ ในชุมชนให้ครอบคลุม ทุกจังหวัด")]');
        await page.click('//button[contains(., "1.2 โครงการขยายผลโครงการบริบาลและคุ้มครองสิทธิผู้สูงอายุ ในชุมชนให้ครอบคลุม ทุกจังหวัด")]');

        // 9️⃣ รอให้เมนูย่อยแสดง แล้วคลิก "บันทึกข้อมูลครั้งที่ 1"
        await page.waitForSelector('a[href="https://volunteer-smart-beta.nu.ac.th/beta-inspectorNew/index.php/Y2568Report08Controller?time_count=1&order=1.2&title=โครงการขยายผลโครงการบริบาลและคุ้มครองสิทธิผู้สูงอายุ ในชุมชนให้ครอบคลุม ทุกจังหวัด"]'); // รอให้ลิงก์โหลด
        await page.click('a[href="https://volunteer-smart-beta.nu.ac.th/beta-inspectorNew/index.php/Y2568Report08Controller?time_count=1&order=1.2&title=โครงการขยายผลโครงการบริบาลและคุ้มครองสิทธิผู้สูงอายุ ในชุมชนให้ครอบคลุม ทุกจังหวัด"]'); // คลิกลิงก์

        console.log('คลิกบันทึกข้อมูลครั้งที่ 1 สำเร็จ!');

        //ส่วนที่ 1 แผนและผลการดำเนินการ
        await page.waitForSelector('input[name="number_caregivers"]');
        await page.fill('input[name="number_caregivers"]', '200');

        await page.waitForSelector('input[name="number_elderly_receiving_care"]');
        await page.fill('input[name="number_elderly_receiving_care"]', '200');

        //ส่วนที่ 2 : ผลการดำเนินงาน
        for (let i = 1; i <= 5; i++) {
            await page.waitForSelector(`input[name="results_by_dimension[${i}][number]"]`);
            await page.fill(`input[name="results_by_dimension[${i}][number]"]`, '20');
        }

        //ส่วนที่ 3 กระบวนการดำเนินงาน
        for (let i = 1; i <= 6; i++) {
