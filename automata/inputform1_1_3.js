...
(async () => {
    ...
    try {
        ...
        //4. กลุ่ม D (1 จังหวัด 1 แห่ง นอกพื้นที่นิคมสร้างตนเอง)

        //การคัดเลือกสถานพัฒนาเด็กปฐมวัย
        await page.waitForSelector('input[name="beyond_settlement_province[check]"][value="true"]');
        await page.waitForSelector('input[name="beyond_settlement_province[check]"][value="false"]');

        // สร้าง Array ของตัวเลือก
        const options = [
            'input[name="beyond_settlement_province[check]"][value="true"]',
            'input[name="beyond_settlement_province[check]"][value="false"]',
        ];
        ...
        //กระบวนการดำเนินงานของกลุ่ม C และ D
        for (let i = 1; i <= 6; i++) {
            await page.waitForSelector(`input[name="beyond_settlement[${i}][check]"]`);
            await page.check(`input[name="beyond_settlement[${i}][check]"]`);
        }
